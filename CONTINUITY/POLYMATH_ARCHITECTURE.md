...
| S3 semantic schema (§10.1: semantic_chunk_type/key_terms/mechanisms/topic_key via Ghost A JSON + heal; topics RETIRED; promote lifts onto children) | **PARTIAL — REAL-DATA GAP (Fable_test audit 2026-07-04)**: prose summary 522/707 + topic_key 702/707 LIVE, but semantic_chunk_type/key_terms/mechanisms = **0/707** — the live summarization model ignores the JSON instruction and the never-fabricate fallback swallows structure silently (fixture model happened to emit JSON). Q2 semantic bonus + Q4 mechanisms lever inert until fixed. Task #35: capture raw Ghost A outputs → JSON mode / strict retry → re-promote lifts retroactively |
