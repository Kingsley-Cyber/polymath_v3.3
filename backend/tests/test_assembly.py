...
def test_grouping_rank_order_and_max_score():
    ...
    pm = {
        ("d1", "p1"): {"text": "P1 FULL", "summary": "p1 sum"},
        ("d2", "p2"): {"text": "P2 FULL", "summary": ""},
        ("d1", "p3"): {"text": "P3 FULL", "summary": "p3 sum"},
    }
...
def test_orphans_and_anchor_lane():
    ...
    pm = {("d1", "p1"): {"text": "P1", "summary": "s"}}
    parents, orphans = assembly.group_parent_candidates(chunks, pm, anchor_doc_ids={"d1"})
...
def test_entity_lines_dedupe_order_cap():
    ...
    prov2 = [
        ...
        {"entity": "Layered Indexing", "relation_family": "graph_payload",
         "definitional_phrase": "index in layers"},
    ]
    lines = assembly.entity_lines_from_chunks(
        [_C("c1", "p", "d", 0.9, provenance=prov1), _C("c2", "p", "d", 0.8, provenance=prov2)]
    )
    ...
    capped = assembly.entity_lines_from_chunks(
        [_C(f"c{i}", "p", "d", 0.5, provenance=[{"entity": f"e{i}"}]) for i in range(20)],
        cap=5,
    )
    assert len(capped) == 5
...
def test_end_to_end_packet_determinism():
    ...
    pm = {
        ("d1", "p1"): {"text": "full text one " * 10, "summary": "sum one"},
        ("d2", "p2"): {"text": "full text two " * 10, "summary": "sum two"},
    }
    parents, orphans = assembly.group_parent_candidates(chunks, pm)
    ...
    assert kinds.index("full") < kinds.index("child") <= kinds.index("entity") - 1 or True
