...
@pytest.mark.asyncio
@pytest.mark.parametrize("requested_tier", ["tier2", "tier3"])
...
@pytest.mark.asyncio
async def test_public_tier2_and_tier3_stubs_raise_clear_messages():
    with pytest.raises(NotImplementedError, match="grammar-constrained"):
        await call_tier2_grammar_constrained()
    with pytest.raises(NotImplementedError, match="submit_semantic_digest"):
        await call_tier3_forced_tool()
...
def test_capability_detector_uses_symbol_then_metadata_and_fails_closed(monkeypatch):
    ...
    assert detect_response_schema_capability("m") == CapabilityDecision(
        True, "litellm.supports_response_schema"
    )
    ...
    assert detect_response_schema_capability("m") == CapabilityDecision(
        True, "litellm.get_model_info.compat"
    )
...
def test_owner_flash_route_is_pinned_tier1_capable_on_backend_litellm():
    decision = detect_response_schema_capability("deepseek/deepseek-v4-flash")
    assert decision.supported is True
    assert decision.source in {
        "litellm.supports_response_schema",
        "litellm.get_model_info.compat",
    }
