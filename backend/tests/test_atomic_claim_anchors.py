...
async def test_claim_anchor_uses_one_bounded_aggregate_and_preserves_sources():
    ...
    assert diagnostics["source_identity_additive"] is True
    assert diagnostics["non_anchor_evidence_additive"] is True
    assert diagnostics["additivity_verified"] is True
...
async def test_sentence_keyed_claim_maps_to_selected_parent_summary_additively():
    ...
    assert anchor["exact_sentence"] == "Feedback changes the operating baseline."
    assert source_additivity_receipt([enriched[0]]) == source_additivity_receipt(
        [source]
    )


def test_source_additivity_receipt_ignores_only_atomic_anchor_metadata():
    source = _source().model_copy(update={"metadata": {"retrieval_lane": "summary"}})
    anchored = source.model_copy(
        update={
            "metadata": {
                **source.metadata,
    ...
    changed_score = source.model_copy(update={"score": 0.89})
    changed_lane = source.model_copy(
        update={"metadata": {"retrieval_lane": "different"}}
    )

    assert source_additivity_receipt([source]) == source_additivity_receipt([anchored])
    assert (
        source_additivity_receipt([source])["non_anchor_evidence_sha256"]
        != source_additivity_receipt([changed_score])["non_anchor_evidence_sha256"]
    )
    assert (
        source_additivity_receipt([source])["non_anchor_evidence_sha256"]
        != source_additivity_receipt([changed_lane])["non_anchor_evidence_sha256"]
    )
    assert (
        source_additivity_receipt([source])["source_ids"]
        != source_additivity_receipt(
            [source.model_copy(update={"chunk_id": "child:other"})]
        )["source_ids"]
    )


@pytest.mark.asyncio
async def test_post_selection_additivity_guard_fails_closed(monkeypatch):
    source = _source()
    original_receipt = source_additivity_receipt
    calls = 0

    def drifting_receipt(sources):
        nonlocal calls
        calls += 1
        receipt = original_receipt(sources)
        if calls >= 2:
            receipt["non_anchor_evidence_sha256"] = "0" * 64
        return receipt

    monkeypatch.setattr(
        "services.retriever.atomic_claim_anchors.source_additivity_receipt",
        drifting_receipt,
    )
    ...
    assert diagnostics["additivity_verified"] is False
    assert diagnostics["anchors_attached"] == 0
    assert diagnostics["candidate_anchors_rejected_additivity"] == 1
    assert diagnostics["reason"] == "post_selection_additivity_invariant_failed"
...
@pytest.mark.parametrize(
    ("raw", "expected"),
    [
        ("you POSITIVE ASSERTED UNTYPED[test]", "You test."),
        (
            "you POSITIVE POSSIBLE UNTYPED[use] dry * | *",
            "You may use dry.",
        ),
        (
            "feedback POSITIVE ASSERTED INFLUENCES operating baseline",
            "Feedback influences operating baseline.",
        ),
        (
            "campaign NEGATIVE ASSERTED USED_FOR retention",
            "Campaign is not used for retention.",
        ),
        (
            "system POSITIVE RECOMMENDED APPLIES_UNDER high load WHEN winter",
            "System should apply under high load; when winter.",
        ),
    ],
)
def test_render_claim_proposition_cleans_known_compiler_grammar(raw, expected):
    original = raw

    rendered = render_claim_proposition(
        raw,
        exact_sentence="Exact source sentence.",
    )

    assert rendered == expected
    assert raw == original
    assert "UNTYPED[" not in rendered
    assert " POSITIVE " not in rendered
    assert " NEGATIVE " not in rendered


def test_render_claim_proposition_falls_back_on_partial_machine_grammar():
    assert (
        render_claim_proposition(
            "broken POSITIVE UNTYPED[test]",
            exact_sentence="The exact evidence remains readable.",
        )
        == "The exact evidence remains readable."
    )


def test_prompt_cleans_claim_text_without_mutating_anchor_metadata(monkeypatch):
    monkeypatch.setattr(
        "services.context_manager.get_settings",
        lambda: SimpleNamespace(
            ATOMIC_CLAIM_ANCHORS_ENABLED=True,
    ...
    raw_claim = "you POSITIVE POSSIBLE UNTYPED[use] dry * | *"
    source = _source().model_copy(
        update={
            "metadata": {
                "atomic_claim_anchors": [
                    {
                        ...
                        "exact_sentence": "You can use dry testing.",
                        "start": 0,
                        "end": 24,
                    }
                ]
    ...
    receipt_before = source_additivity_receipt([source])

    prompt = context_manager.build_augmented_prompt("dry testing", [source])

    assert 'candidate claim "You may use dry."' in prompt
    assert raw_claim not in prompt
    assert source.metadata["atomic_claim_anchors"][0]["claim_text"] == raw_claim
    assert source_additivity_receipt([source]) == receipt_before
