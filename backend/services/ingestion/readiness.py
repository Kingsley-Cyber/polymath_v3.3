...
READINESS_SCHEMA_VERSION = "corpus_readiness.v2"
...
SOURCE_KEY_IDENTITY_FIELDS: tuple[str, ...] = (
    "source_key",
...
def _nonempty_text_expr(field: str) -> dict[str, Any]:
    """Mongo expression that treats missing, null, and empty text alike."""
...
def _has_any_identity_field_query(fields: tuple[str, ...]) -> dict[str, Any]:
    return {"$or": [{field: {"$exists": True, "$nin": [None, ""]}} for field in fields]}
...
def neo4j_pressure_from_graph_promotion_jobs(
    ...
    *,
    ...
    queue_depth = sum(
        _int((graph_jobs or {}).get(status)) for status in ("queued", "running")
    )
...
def build_corpus_readiness_snapshot(
    *,
    ...
    lexicon_tracked = _int(document_counts.get("lexicon_tracked"))
    lexicon_ready = _int(document_counts.get("lexicon_ready"))
    lexicon_pending = max(doc_total - lexicon_ready, 0) if lexicon_tracked > 0 else 0
    if lexicon_tracked > 0:
        fully_enriched = min(fully_enriched, lexicon_ready)
    ...
    retrieval_parent_total = _int(
        summary_counts.get(
            "retrieval_parent_total", summary_counts.get("body_parent_total")
        )
    )
    retrieval_parent_done = _int(
        summary_counts.get(
            "retrieval_parent_done", summary_counts.get("body_parent_done")
        )
    )
    body_parent_total = _int(
        summary_counts.get("body_parent_total", retrieval_parent_total)
    )
    body_parent_done = _int(
        summary_counts.get("body_parent_done", retrieval_parent_done)
    )
    ...
    summary_tree_index_tracked = _int(summary_counts.get("summary_tree_index_tracked"))
    summary_tree_index_ready = _int(summary_counts.get("summary_tree_index_ready"))
    summary_tree_index_pending = (
        max(document_tree_done - summary_tree_index_ready, 0)
        if summary_tree_index_tracked > 0
        else 0
    )
    if summary_tree_index_tracked > 0:
        fully_enriched = min(fully_enriched, summary_tree_index_ready)
    document_profile_index_tracked = _int(
        summary_counts.get("document_profile_index_tracked")
    )
    document_profile_index_ready = _int(
        summary_counts.get("document_profile_index_ready")
    )
    document_profile_index_pending = (
        max(document_profile_done - document_profile_index_ready, 0)
        if document_profile_index_tracked > 0
        else 0
    )
    if document_profile_index_tracked > 0:
        fully_enriched = min(fully_enriched, document_profile_index_ready)
    ...
    stale_failure_reason_counts = {
        str(key): _int(value)
        for key, value in (
            graph_counts.get("stale_failure_reason_counts") or {}
        ).items()
    }
    reconciled_stale_failure_docs = _int(
        graph_counts.get("reconciled_stale_failure_docs")
    )
    reconciled_stale_failure_rows = _int(
        graph_counts.get("reconciled_stale_failure_rows")
    )
    ...
    source_parse_pending_jobs = sum(
        _int(source_parse_jobs.get(status))
        for status in SOURCE_PARSE_PENDING_JOB_STATUSES
    )
    source_parse_failed_jobs = sum(
        _int(source_parse_jobs.get(status))
        for status in SOURCE_PARSE_FAILED_JOB_STATUSES
    )
    extraction_pending_jobs = sum(
        _int(extraction_jobs.get(status)) for status in EXTRACTION_PENDING_JOB_STATUSES
    )
    extraction_failed_jobs = sum(
        _int(extraction_jobs.get(status)) for status in EXTRACTION_FAILED_JOB_STATUSES
    )
    extraction_blocked_jobs = sum(
        _int(extraction_jobs.get(status)) for status in EXTRACTION_BLOCKED_JOB_STATUSES
    )
    summary_waiting_jobs = sum(
        _int(summary_jobs.get(status)) for status in SUMMARY_WAITING_JOB_STATUSES
    )
    summary_pending_jobs = (
        sum(_int(summary_jobs.get(status)) for status in SUMMARY_PENDING_JOB_STATUSES)
        + summary_waiting_jobs
    )
    summary_failed_jobs = sum(
        _int(summary_jobs.get(status)) for status in SUMMARY_FAILED_JOB_STATUSES
    )
    ...
    duplicate_source_key_groups = _int(
        idempotency_counts.get("duplicate_source_key_groups")
    )
    duplicate_source_key_docs = _int(
        idempotency_counts.get("duplicate_source_key_docs")
    )
    source_key_collision_groups = _int(
        idempotency_counts.get("source_key_collision_groups")
    )
    source_key_collision_docs = _int(
        idempotency_counts.get("source_key_collision_docs")
    )
    duplicate_content_hash_groups = _int(
        idempotency_counts.get("duplicate_content_hash_groups")
    )
    duplicate_content_hash_docs = _int(
        idempotency_counts.get("duplicate_content_hash_docs")
    )
    ...
    missing_stage_identity = _int(
        idempotency_counts.get("stage_identity_missing_total")
    )
    ...
    if lexicon_pending:
        blocking.append("corpus_lexicon_pending")
    if summary_tree_index_pending:
        blocking.append("summary_tree_index_pending")
    if document_profile_index_pending:
        blocking.append("document_profile_index_pending")
    ...
    if doc_total == 0:
        ...
        status = "needs_repair"
    elif extraction_pending_jobs and not (
        failed_docs or graph_failed_docs or failed_chunks or extraction_failed_jobs
    ):
        ...
        retrieval_missing
        or document_sync_missing
        or summary_tree_index_pending
        or document_profile_index_pending
    ):
        status = "summaries_pending"
    elif lexicon_pending:
    ...
    if document_pipeline_failed_jobs:
        next_actions.append(
            _readiness_action(
                ...
                blocked_by_pressure=backpressure.get("document_pipeline_allowed")
                is False,
            )
        )
    if document_pipeline_pending_jobs:
    ...
    if failed_docs or graph_failed_docs or failed_chunks or extraction_failed_jobs:
        next_actions.append(
            _readiness_action(
                ...
                blocked_by_pressure=backpressure.get("extraction_backfill_allowed")
                is False,
            )
        )
    if extraction_blocked_jobs:
    ...
    if summary_failed_jobs:
        next_actions.append(
            _readiness_action(
                ...
                blocked_by_pressure=backpressure.get("summary_backfill_allowed")
                is False,
            )
        )
    ...
    if graph_pending:
        next_actions.append(
            _readiness_action(
                ...
                count=graph_pending,
                blocked_by_pressure=backpressure.get("graph_promotion_allowed")
                is False,
            )
        )
    if exact_duplicate_source_key_groups or duplicate_content_hash_groups:
    ...
    if lexicon_pending:
        next_actions.append(
            _readiness_action(
                action_id="backfill_corpus_lexicon",
                label="Backfill corpus vocabulary",
                lane="lexicon",
                severity="warning",
                reason=(
                    "Queryable documents still need the versioned corpus vocabulary "
                    "projection used by grounded query translation."
                ),
                count=lexicon_pending,
            )
        )
    if summary_tree_index_pending:
        next_actions.append(
            _readiness_action(
                action_id="backfill_summary_tree_index",
                label="Index summary hierarchy",
                lane="summary_index",
                severity="warning",
                reason=(
                    "Document summary trees exist in Mongo but their section/rollup "
                    "routing vectors are not fully mirrored into Qdrant."
                ),
                count=summary_tree_index_pending,
            )
        )
    ...
    return {
        ...
        "documents": {
            ...
            "lexicon_tracked": lexicon_tracked,
            "lexicon_ready": lexicon_ready,
            "lexicon_pending": lexicon_pending,
            "lexicon_coverage": _coverage(lexicon_ready, doc_total),
        ...
        "summaries": {
            ...
            "summary_tree_index_tracked": summary_tree_index_tracked,
            "summary_tree_index_ready": summary_tree_index_ready,
            "summary_tree_index_pending": summary_tree_index_pending,
            "summary_tree_index_coverage": _coverage(
                summary_tree_index_ready,
                document_tree_done,
            ),
            "document_profile_index_tracked": document_profile_index_tracked,
            "document_profile_index_ready": document_profile_index_ready,
            "document_profile_index_pending": document_profile_index_pending,
            "document_profile_index_coverage": _coverage(
                document_profile_index_ready,
        ...
        "graph": {
            ...
            "stale_failure_reason_counts": dict(
                sorted(stale_failure_reason_counts.items())
            ),
            "stale_failure_scan_limit": _int(
                graph_counts.get("stale_failure_scan_limit")
            ),
            "stale_failure_scan_limited": bool(
                graph_counts.get("stale_failure_scan_limited")
            ),
        ...
        "idempotency": {
            "source_keyed_documents": _int(
                idempotency_counts.get("source_keyed_documents")
            ),
            "content_hash_documents": _int(
                idempotency_counts.get("content_hash_documents")
            ),
        ...
        "pressure": pressure or pressure_snapshot,
    }
...
async def compute_corpus_readiness(db: Any, corpus_id: str) -> dict[str, Any]:
                ...
                {"$match": doc_match},
                {
                    "$group": {
                                ...
                                "$cond": [
                                    {
                                        "$and": [
                                            {
                                                "$eq": [
                                                    "$write_state.mongo_written",
                                                    True,
                                                ]
                                            },
                                            {
                                                "$eq": [
                                                    "$write_state.qdrant_written",
                                                    True,
                                                ]
                                            },
                                        ]
                        ...
                        "graph_promoted": {
                            "$sum": {
                                "$cond": [
                                    {"$eq": ["$write_state.neo4j_written", True]},
                                    1,
                                    0,
                                ]
                            }
                        ...
                        "summary_indexed": {
                            "$sum": {
                                "$cond": [
                                    {"$eq": ["$write_state.summaries_indexed", True]},
                                    1,
                                    0,
                                ]
                            }
                        ...
                        "failed_chunks": {
                            "$sum": {"$ifNull": ["$ghost_b_failure_count", 0]}
                        },
                        "failure_docs": {
                            "$sum": {
                                "$cond": [
                                    {
                                        "$gt": [
                                            {"$ifNull": ["$ghost_b_failure_count", 0]},
                                            0,
                                        ]
                                    },
                                    1,
                        ...
                        "lexicon_ready": {
                            "$sum": {
                                "$cond": [
                                    {"$eq": ["$lexicon_state", "lexicon_ready"]},
                                    1,
                                    0,
                                ]
                            }
    ...
    stage_rows = (
        ...
        .aggregate(
            [
                ...
                {
                    "$group": {
                        "_id": {"$ifNull": ["$ingest_stage", "unknown"]},
                        "count": {"$sum": 1},
                    }
                },
            ]
    ...
    chunk_total = await db["chunks"].count_documents(
    ...
    chunk_counts = {
        "total": chunk_total,
        "docs_with_chunks": _int(
            chunk_doc_rows[0].get("count") if chunk_doc_rows else 0
        ),
    }
    ...
    body_parent_query = with_active_records(
        {"corpus_id": corpus_id, "$and": [BODY_PARENT_CLAUSE]}
    )
    parent_done_query = with_active_records(
        {"corpus_id": corpus_id, **SUMMARY_TEXT_CLAUSE}
    )
    ...
    document_summary_rows = (
        ...
        .aggregate(
            [
                ...
                {
                    "$project": {
                        "profile_done": _nonempty_text_expr(
                            "$doc_profile.summary"
                        ),
                        "tree_done": {"$gt": [{"$size": "$document_summary_tree"}, 0]},
                        "tree_index_tracked": {
                            "$in": [
                        ...
                        "profile_index_tracked": {
                            "$in": [
                                "$write_state.document_profile_indexed",
                                [True, False],
                            ]
                        },
                        "profile_index_ready": {
                ...
                },
                {
                    "$group": {
                        ...
                        "document_tree_done": {"$sum": {"$cond": ["$tree_done", 1, 0]}},
                        "document_both_done": {
                            "$sum": {
                                "$cond": [
                        ...
                        "summary_tree_index_tracked": {
                            "$sum": {"$cond": ["$tree_index_tracked", 1, 0]}
                        },
                        "summary_tree_index_ready": {
                            "$sum": {"$cond": ["$tree_index_ready", 1, 0]}
                        },
                        "document_profile_index_tracked": {
                            "$sum": {"$cond": ["$profile_index_tracked", 1, 0]}
                        },
                        "document_profile_index_ready": {
                            "$sum": {"$cond": ["$profile_index_ready", 1, 0]}
                        },
                    }
                },
            ]
    ...
    retrieval_parent_total = await db["parent_chunks"].count_documents(
        retrieval_parent_query
    )
    ...
    summary_counts = {
        ...
        "retrieval_parent_total": retrieval_parent_total,
        "retrieval_parent_done": retrieval_parent_done,
        "body_parent_total": await db["parent_chunks"].count_documents(
            body_parent_query
        ),
        "body_parent_done": await db["parent_chunks"].count_documents(
            body_parent_done_query
        ),
        "document_done": _int(document_summary_counts.get("document_done")),
        "document_profile_done": _int(
            document_summary_counts.get("document_profile_done")
        ),
        ...
        "document_profile_only": _int(
            document_summary_counts.get("document_profile_only")
        ),
        ...
        "summary_tree_index_tracked": _int(
            document_summary_counts.get("summary_tree_index_tracked")
        ),
        "summary_tree_index_ready": _int(
            document_summary_counts.get("summary_tree_index_ready")
        ),
        "document_profile_index_tracked": _int(
            document_summary_counts.get("document_profile_index_tracked")
        ),
        "document_profile_index_ready": _int(
            document_summary_counts.get("document_profile_index_ready")
        ),
    }
    ...
    unmarked_promoted_extraction_rows = (
        await db["ghost_b_extractions"]
        .aggregate(
            [
                {
    ...
    source_key_query = with_active_records(
        {
            "corpus_id": corpus_id,
            **_has_any_identity_field_query(SOURCE_KEY_IDENTITY_FIELDS),
        }
    )
    content_hash_query = with_active_records(
        {
            "corpus_id": corpus_id,
            **_has_any_identity_field_query(CONTENT_HASH_IDENTITY_FIELDS),
        }
    )
    ...
    async def _duplicate_identity_rows(
        ...
        *,
        ...
        pipeline: list[dict[str, Any]] = [
            ...
            {
                "$addFields": {
                    "_identity_group_key": _first_non_empty_identity_expr(fields)
                }
            },
        ]
        if classify_content_collisions:
            pipeline.append(
                {"$addFields": {"_identity_content_hash": content_hash_expr}}
            )
        pipeline.extend(
            [
                {
                    "$match": {
                        "_identity_group_key": {"$exists": True, "$nin": [None, ""]}
                    }
                },
                {"$group": group_stage},
                {"$match": {"docs": {"$gt": 1}}},
        ...
        if classify_content_collisions:
            pipeline.extend(
                [
                    ...
                    {
                        "$addFields": {
                            "_content_hash_count": {"$size": "$_content_hashes"}
                        }
                    },
                    {
                        "$addFields": {
                            "_source_key_collision": {
                                "$gt": ["$_content_hash_count", 1]
                            }
                        }
                    },
                ]
    ...
    duplicate_content_hash_rows = await _duplicate_identity_rows(
        CONTENT_HASH_IDENTITY_FIELDS
    )
    ...
    extraction_jobs_missing_stage_identity = await _stage_identity_missing(
        "extraction_jobs"
    )
    ...
    idempotency_counts = {
        "source_keyed_documents": await db["documents"].count_documents(
            source_key_query
        ),
        "content_hash_documents": await db["documents"].count_documents(
            content_hash_query
        ),
        ...
        "duplicate_source_key_groups": _int(
            duplicate_source_key_rows[0].get("groups")
        ...
        "duplicate_content_hash_groups": _int(
            duplicate_content_hash_rows[0].get("groups")
            if duplicate_content_hash_rows
            else 0
        ),
        "duplicate_content_hash_docs": _int(
            duplicate_content_hash_rows[0].get("docs")
            if duplicate_content_hash_rows
            else 0
        ),
    }
    ...
    stale_graph_job_rows = (
        ...
        .aggregate(
            [
                ...
                {
                    "$addFields": {
                        "_readiness_doc": {"$arrayElemAt": ["$readiness_doc", 0]}
                    }
                },
                {
                    "$match": {
    ...
    source_parse_job_rows = (
        await db["source_parse_jobs"]
        .aggregate(
            [
                {"$match": {"corpus_id": corpus_id}},
                {"$group": {"_id": "$status", "count": {"$sum": 1}}},
    ...
    extraction_jobs = {
        str(row["_id"]): int(row["count"]) for row in extraction_job_rows
    }
    try:
    ...
    document_pipeline_job_rows = (
        await db["document_pipeline_jobs"]
        .aggregate(
            [
                {"$match": {"corpus_id": corpus_id}},
                {"$group": {"_id": "$status", "count": {"$sum": 1}}},
    ...
    try:
        ...
        qdrant_write_concurrency = int(
            getattr(settings, "QDRANT_INGEST_WRITE_CONCURRENCY", 2)
        )
        neo4j_write_concurrency = int(
            getattr(settings, "NEO4J_INGEST_WRITE_CONCURRENCY", 1)
        )
    ...
    try:
        scheduler_state = (
            await db["ingest_scheduler_state"].find_one(
                {"_id": corpus_id},
                {
                    "_id": 0,
                    "idle_ticks": 1,
...
async def get_materialized_corpus_readiness(
