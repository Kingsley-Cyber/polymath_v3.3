...
_ANCHOR_METADATA_KEY = "atomic_claim_anchors"


def source_additivity_receipt(sources: list[SourceChunk]) -> dict[str, Any]:
    """Fingerprint final evidence after removing only atomic-anchor metadata."""
    ...
    for source in list(sources or []):
        source_ids.append(
            [
                ...
                str(source.parent_id or ""),
            ]
        )
        payload = source.model_dump(mode="json")
        metadata = dict(payload.get("metadata") or {})
        metadata.pop(_ANCHOR_METADATA_KEY, None)
    ...
    return {
        ...
        "source_ids_sha256": hashlib.sha256(ids_bytes).hexdigest(),
        "non_anchor_evidence_sha256": hashlib.sha256(evidence_bytes).hexdigest(),
        "non_anchor_evidence_bytes": len(evidence_bytes),
    }


def _record_additivity(
    diagnostics: dict[str, Any],
    *,
    before: dict[str, Any],
    after: dict[str, Any],
) -> bool:
    identity_equal = before["source_ids"] == after["source_ids"]
    evidence_equal = (
        before["non_anchor_evidence_sha256"] == after["non_anchor_evidence_sha256"]
        and before["non_anchor_evidence_bytes"] == after["non_anchor_evidence_bytes"]
    )
    diagnostics.update(
        {
            "source_ids_sha256_before": before["source_ids_sha256"],
            "source_ids_sha256_after": after["source_ids_sha256"],
            "non_anchor_evidence_sha256_before": before["non_anchor_evidence_sha256"],
            "non_anchor_evidence_sha256_after": after["non_anchor_evidence_sha256"],
            "non_anchor_evidence_bytes_before": before["non_anchor_evidence_bytes"],
            "non_anchor_evidence_bytes_after": after["non_anchor_evidence_bytes"],
            "source_identity_additive": identity_equal,
            "non_anchor_evidence_additive": evidence_equal,
            "additivity_verified": identity_equal and evidence_equal,
        }
    )
    return bool(identity_equal and evidence_equal)
...
async def attach_atomic_claim_anchors(
    ...
    *,
    ...
    additivity_before = source_additivity_receipt(ordered_sources)
    ...
    if not unique_keys or not query_terms:
        ...
        _record_additivity(
            diagnostics,
            before=additivity_before,
            after=source_additivity_receipt(ordered_sources),
        )
    ...
    for source in ordered_sources:
        ...
        metadata[_ANCHOR_METADATA_KEY] = selected
    ...
    additivity_after = source_additivity_receipt(enriched)
    if not _record_additivity(
        diagnostics,
        before=additivity_before,
        after=additivity_after,
    ):
        rejected = diagnostics["anchors_attached"]
        diagnostics.update(
            {
                "reason": "post_selection_additivity_invariant_failed",
                "candidate_anchors_rejected_additivity": rejected,
                "anchors_attached": 0,
                "sources_anchored": 0,
            }
        )
