...
async def _run(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        score = (
            {
                "status": "unscored",
                "counts": {
                    "gold_positive": len(gold["positive_canonical_triples"]),
                    "neo4j_promoted_positive": len(promoted),
                    "qualified_gold": len(gold.get("qualified_statements", [])),
                    "qualified_retained": len(qualified),
                },
                "metrics": {},
                "gates": {},
                "comparisons": [],
            ...
            else _score(gold=gold, promoted=promoted, qualified=qualified)
        )
        ...
        result_name = "extraction_receipt.json" if args.extract_only else "score.json"
        ...
        return 0 if args.extract_only or score["status"] == "passed" else 1
    finally:
        await stores.close()
