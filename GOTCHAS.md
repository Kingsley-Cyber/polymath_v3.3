...
**Re-running the installer is now safe.** `scripts/install_apple_mlx_runtime.sh` explicitly excludes the three sidecar `main.py` files from rsync when a host copy already exists at `~/PolymathRuntime/apple_ml_services/{embedder_mlx,reranker_mlx,docling_svc}/main.py`. So if you've replaced the scaffold with your verified implementation on Mac Studio, re-running the installer to refresh the venv / model cache / LaunchAgent will NOT clobber it. The installer logs `[apple-mlx] protecting verified host file: <rel>` for each preserved file. To force a full re-sync (drop your verified code and restore the scaffold), delete the host files first.
